            ...
            with structlog.contextvars.bound_contextvars(
                **request.ctx, dev_mode=cfg.dev_mode
                ...
                match request:
                    ...
                    case EpisodeRequest():
                        tg.start_soon(
                            handle_episode_request, request, tc, rdbc, sync, repo, rc
                        )
                    case _:
...
async def runit(loop: bool = True):
    ...
    async with anyio.create_task_group() as tg:
        ...
        if loop:
            tg.start_soon(periodic_send, send_stream.clone(), "process", 300)
            tg.start_soon(periodic_send, send_stream.clone(), "update", 300)
